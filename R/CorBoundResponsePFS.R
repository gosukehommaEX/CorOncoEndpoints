...
#'
#' **Key differences from \code{\link{CorBoundResponseTTE}}:**
#' \itemize{
...
#'
#' @export
CorBoundResponsePFS <- function(p, hazard_OS, hazard_PFS) {
  # Input validation
  ...
  if (hazard_ratio > 10) {
    warning("hazard_PFS/hazard_OS ratio (", round(hazard_ratio, 2), 
            ...
            "which may result in very narrow correlation bounds close to zero.")
  }
  
  # Calculate power term for bounds calculation
  # This can become numerically unstable for extreme hazard ratios
  power_exponent <- hazard_TTP / hazard_OS
  
  # Calculate correlation bounds using Fréchet-Hoeffding copula bounds
  # Lower bound: using countermonotonic copula between OS and Response
  lower_power_term <- (1 - p) ^ power_exponent
  if (!is.finite(lower_power_term)) {
    warning("Numerical instability detected in lower bound calculation. ",
            "This may occur with extreme parameter combinations.")
    lower_power_term <- 0
  }
  rho_lower <- sqrt((1 - p) / p) * (hazard_OS / hazard_TTP) * (lower_power_term - 1)
  
  # Upper bound: using comonotonic copula between OS and Response
  upper_power_term <- p ^ power_exponent
  if (!is.finite(upper_power_term)) {
    warning("Numerical instability detected in upper bound calculation. ",
            "This may occur with extreme parameter combinations.")
    upper_power_term <- 0
  }
  rho_upper <- sqrt(p / (1 - p)) * (hazard_OS / hazard_TTP) * (1 - upper_power_term)
  
  rho_bound <- c(rho_lower, rho_upper)
  
  # Final validation that bounds are sensible
  if (abs(rho_lower) > 1 || abs(rho_upper) > 1) {
    warning("Calculated bounds exceed [-1, 1]. This indicates numerical issues. ",
